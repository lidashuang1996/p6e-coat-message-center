...
public class Launcher {
    ...
    private LauncherData execute(LauncherSource source, Map<String, String> data, List<String> recipients) {
        ...
        final TemplateData templateData = template(source, data);
        final ConfigData configData = configParser.execute(launcherPatternParser.execute(source));
        if (source.type().equalsIgnoreCase(templateData.type())
            ...
            if (MAIL_TYPE.equalsIgnoreCase(configData.type())) {
                return mailLauncherParser.execute((MailConfigData) configData, templateData, recipients);
            }
            if (SMS_TYPE.equalsIgnoreCase(configData.type())) {
                return shortMessageLauncherParser.execute((ShortMessageConfigData) configData, templateData, recipients);
            }
            if (MMS_TYPE.equalsIgnoreCase(configData.type())) {
                return mobileMessageLauncherParser.execute((MobileMessageConfigData) configData, templateData, recipients);
            }
        }
     ...
     * @return 模板数据对象
     */
    private TemplateData template(LauncherSource source, Map<String, String> data) {
        if (source == null) {
            throw new NullPointerException("launcher source is null");
        }
        ...
        final TemplateSource templateSource = dataSourceFactory.getTemplateSource(source.template());
